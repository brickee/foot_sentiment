...
fh = logging.FileHandler('./logging_foot_turning_away.log')
...
def get_frames(predict_videoFile = None):
    ...
    if not predict_videoFile:
        for (dirpath, dirnames, filenames) in os.walk('./turning/marked_only_videos/'):
            for file in filenames:
                ...
                while(cap.isOpened()):
                    ...
                    out_dir = os.path.join('./turning/marked_only_images/', file)
    ...
    if not predic_frames:
        for (dirpath, dirnames, filenames) in os.walk('./turning/marked_only_videos/'):
            for file in filenames:
                ...
                # Manual Annotation
                if file == 'foot_turning_away_test_1.markonly.mp4':
                    y = [0 for _ in range(1, 26)]
                    y.extend([1 for _ in range(26, 41)])
                    y.extend([0 for _ in range(41, len(X) + 1)])

                elif file == 'foot_turning_away_test_2.markonly.mp4':
                    y = [0 for _ in range(1, 7)]
                    y.extend([1 for _ in range(7, 24)])
                    y.extend([0 for _ in range(24, len(X) + 1)])

                elif file == 'foot_turning_away_test_3.markonly.mp4':
                    y = [0 for _ in range(1, 21)]
                    y.extend([1 for _ in range(21, 46)])
                    y.extend([0 for _ in range(46, len(X) + 1)])




                elif file == 'foot_turning_away_train_1.markonly.mp4':
                    y = [0 for _ in range(1, 16)]
                    y.extend([1 for _ in range(16, 47)])
                    y.extend([0 for _ in range(47, len(X) + 1)])

                elif file == 'foot_turning_away_train_2.markonly.mp4':
                    y = [0 for _ in range(1, 25)]
                    y.extend([1 for _ in range(25, 43)])
                    y.extend([0 for _ in range(43, len(X) + 1)])

                elif file == 'foot_turning_away_train_3.markonly.mp4':
                    y = [0 for _ in range(1, 4)]
                    y.extend([1 for _ in range(4, 21)])
                    y.extend([0 for _ in range(21, len(X) + 1)])

                elif file == 'foot_turning_away_train_4.markonly.mp4':
                    y = [0 for _ in range(1, 9)]
                    y.extend([1 for _ in range(9, 23)])
                    y.extend([0 for _ in range(23, len(X) + 1)])

                elif file == 'foot_turning_away_train_5.markonly.mp4':
                    y = [0 for _ in range(1, 13)]
                    y.extend([1 for _ in range(13, 34)])
                    y.extend([0 for _ in range(34, len(X) + 1)])

                elif file == 'foot_turning_away_train_6.markonly.mp4':
                    y = [0 for _ in range(1, 6)]
                    y.extend([1 for _ in range(6, 33)])
                    y.extend([0 for _ in range(33, len(X) + 1)])

                elif file == 'foot_turning_away_train_7.markonly.mp4':
                    y = [1 for _ in range(1, 16)]
                    y.extend([0 for _ in range(16, len(X) + 1)])

                elif file == 'foot_turning_away_train_8.markonly.mp4':
                    y = [0 for _ in range(1, 10)]
                    y.extend([1 for _ in range(10, 32)])
                    y.extend([0 for _ in range(32, len(X) + 1)])

                elif file == 'foot_turning_away_train_9.markonly.mp4':
                    y = [0 for _ in range(1, 23)]
                    y.extend([1 for _ in range(23,47 )])
                    y.extend([0 for _ in range(47, len(X) + 1)])

                elif file == 'foot_turning_away_train_10.markonly.mp4':
                    y = [0 for _ in range(1, 37)]
                    y.extend([1 for _ in range(37,53 )])
                    y.extend([0 for _ in range(53, len(X) + 1)])


                elif file == 'foot_turning_away_train_11.markonly.mp4':
                    y = [0 for _ in range(1, 101)]
                    y.extend([1 for _ in range(101, 130)])
                    y.extend([0 for _ in range(130, len(X) + 1)])

                elif file == 'foot_turning_away_train_12.markonly.mp4':
                    y = [0 for _ in range(1, 108)]
                    y.extend([1 for _ in range(108, 141)])
                    y.extend([0 for _ in range(141, len(X) + 1)])

                elif file == 'foot_turning_away_train_13.markonly.mp4':
                    y = [0 for _ in range(1, 75)]
                    y.extend([1 for _ in range(75, 91)])
                    y.extend([0 for _ in range(91, len(X) + 1)])

                elif file == 'foot_turning_away_train_14.markonly.mp4':
                    y = [0 for _ in range(1, 111)]
                    y.extend([1 for _ in range(111, 130)])
                    y.extend([0 for _ in range(130, len(X) + 1)])
                ...
                assert len(X) == len(y)
                out_dir = './turning/marked_only_data'
                if not os.path.exists(out_dir):
                    os.makedirs(out_dir)
                y = np.array(y)
                ...
                if 'test' in file:
                    ...
                    plt.plot(x_axis, y)
                    plt.savefig('./turning/' + file.split('.')[0] + '_gold_label.png')
                    plt.close()
        ...
        for count,img in enumerate(predic_frames):
            if img.shape != (480, 272, 3):
                ...
                img = cv2.resize(img, (272, 480))
                # logger.info(img.shape)
                # cv2.imwrite('./withdrawing/sample%d.jpg'%(len(predic_frames)), img)
                X.append(img)  # storing each image in array X
...
def Train_LRCN():
    ...
    for m in range(1, train_count + 1):
        frames, _ = get_frames(predict_videoFile='./turning/marked_only_videos/%s_train_%d.markonly.mp4'% (data_type, m))
        assert frames
        X = prepare_data(frames)
        y = pickle.load(open('./turning/marked_only_data/y_%s_train_%d' % (data_type, m), 'rb'))
        assert len(X) == len(y)
    ...
    for m in range(1, test_count + 1):
        frames, _ = get_frames(predict_videoFile='./turning/marked_only_videos/%s_test_%d.markonly.mp4'% (data_type, m))
        assert frames
        X = prepare_data(frames)
        y = pickle.load(open('./turning/marked_only_data/y_%s_test_%d' % (data_type, m), 'rb'))
        assert len(X) == len(y)
        # X = diff_adjacent_frame(X)
        X_test.append(torch.tensor(X,dtype=torch.float))
        y_test.append(torch.tensor(y,dtype=torch.long))

    # Add more negative test samples
    for m in range(1, 3):
...
def Predict_LRCN(data_type = 'foot_turning_away' ,test_video='./turning/marked_only_videos/foot_turning_away_test_1.markonly.mp4'):
    ...
    # logger.info('Saving results to timeLabel.json')
    # with open('./test_timeLabel.json','a') as f:
    ...
    plt.ylim(-0.1, 1.1)
    plt.savefig('./turning/%s_timeLabel_%s.png'%(file_name,data_type))
    # plt.show(block=True)
    # plt.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--type', default='foot_turning_away',
                        help='3 options: foot_withdrawing, foot_pacing, foot_turning_away')
    parser.add_argument('--video', default='./turning/marked_only_videos/foot_turning_away_test_3.markonly.mp4',
                        help='path to test video')
    # parser.add_argument('--video', default='./test_marked_only/4400_2.markOnly.mp4',
    #                     help='path to test video')
