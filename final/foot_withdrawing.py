...
fh = logging.FileHandler('./logging_foot_withdrawing.log')
...
def get_frames(predict_videoFile = None):
    ...
    if not predict_videoFile:
        for (dirpath, dirnames, filenames) in os.walk('./withdrawing/marked_only_videos/'):
            for file in filenames:
                ...
                while(cap.isOpened()):
                    ...
                    out_dir = os.path.join('./withdrawing/marked_only_images/', file)
    ...
    if not predic_frames:
        for (dirpath, dirnames, filenames) in os.walk('./withdrawing/marked_only_videos/'):
            for file in filenames:
                ...
                # Manual Annotation
                if file == 'foot_withdrawing_test_1.markonly.mp4':
                    y = [0 for _ in range(1, 29)]
                    y.extend([1 for _ in range(29, len(X)+1)])

                elif file == 'foot_withdrawing_test_2.markonly.mp4':
                    y = [0 for _ in range(1, 28)]
                    y.extend([1 for _ in range(28, 42)])
                    y.extend([0 for _ in range(42, len(X)+1)])

                elif file == 'foot_withdrawing_test_3.markonly.mp4':
                    y = [0 for _ in range(1, 8)]
                    y.extend([1 for _ in range(8, 22)])
                    y.extend([0 for _ in range(22, len(X)+1)])

                elif file == 'foot_withdrawing_test_4.markonly.mp4':
                    y = [0 for _ in range(1, 55)]
                    y.extend([1 for _ in range(55, 82)])
                    y.extend([0 for _ in range(82, len(X)+1)])


                elif file == 'foot_withdrawing_train_1.markonly.mp4':
                    y = [0 for _ in range(1, 10)]
                    y.extend([1 for _ in range(10, 27)])
                    y.extend([0 for _ in range(27, len(X)+1)])

                elif file == 'foot_withdrawing_train_2.markonly.mp4':
                    y = [1 for _ in range(1, 17)]
                    y.extend([0 for _ in range(17, len(X)+1)])

                elif file == 'foot_withdrawing_train_3.markonly.mp4':
                    y = [0 for _ in range(1, 5)]
                    y.extend([1 for _ in range(5, 37)])
                    y.extend([0 for _ in range(37, len(X) + 1)])

                elif file == 'foot_withdrawing_train_4.markonly.mp4':
                    y = [0 for _ in range(1, 33)]
                    y.extend([1 for _ in range(33, 61)])
                    y.extend([0 for _ in range(61, len(X) + 1)])

                elif file == 'foot_withdrawing_train_5.markonly.mp4':
                    y = [0 for _ in range(1, 5)]
                    y.extend([1 for _ in range(5, 36)])
                    y.extend([0 for _ in range(36, len(X) + 1)])

                elif file == 'foot_withdrawing_train_6.markonly.mp4':
                    y = [0 for _ in range(1, 9)]
                    y.extend([1 for _ in range(9, 34)])
                    y.extend([0 for _ in range(34, len(X) + 1)])

                elif file == 'foot_withdrawing_train_7.markonly.mp4':
                    y = [0 for _ in range(1, 29)]
                    y.extend([1 for _ in range(29,len(X) + 1)])

                elif file == 'foot_withdrawing_train_8.markonly.mp4':
                    y = [0 for _ in range(1, 21)]
                    y.extend([1 for _ in range(21, len(X) + 1)])

                elif file == 'foot_withdrawing_train_9.markonly.mp4':
                    y = [0 for _ in range(1, 26)]
                    y.extend([1 for _ in range(26, len(X) + 1)])

                elif file == 'foot_withdrawing_train_10.markonly.mp4':
                    y = [0 for _ in range(1, 174)]
                    y.extend([1 for _ in range(174, 214)])
                    y.extend([0 for _ in range(214, 297)])
                    y.extend([1 for _ in range(297, len(X) + 1)])

                elif file == 'foot_withdrawing_train_11.markonly.mp4':
                    y = [0 for _ in range(1, 150)]
                    y.extend([1 for _ in range(150, 171)])
                    y.extend([0 for _ in range(171, 310)])
                    y.extend([1 for _ in range(310, 348)])
                    y.extend([0 for _ in range(348, len(X) + 1)])

                elif file == 'foot_withdrawing_train_12.markonly.mp4':
                    y = [0 for _ in range(1, 161)]
                    y.extend([1 for _ in range(161, 192)])
                    y.extend([0 for _ in range(192, 298)])
                    y.extend([1 for _ in range(298, 325)])
                    y.extend([0 for _ in range(325, len(X) + 1)])
                ...
                assert len(X) == len(y)
                out_dir = './withdrawing/marked_only_data'
                if not os.path.exists(out_dir):
                    os.makedirs(out_dir)
                y = np.array(y)
                ...
                if 'test' in file:
                    ...
                    plt.plot(x_axis, y)
                    plt.savefig('./withdrawing/' + file.split('.')[0] + '_gold_label.png')
                    plt.close()
...
def Train_LRCN():
    ...
    for m in range(1, train_count + 1):
        frames, _ = get_frames(predict_videoFile='./%s/marked_only_videos/%s_train_%d.markonly.mp4'% (data_type.split('_')[-1],data_type, m))
        assert frames
        X = prepare_data(frames)
        y = pickle.load(open('./%s/marked_only_data/y_%s_train_%d' % (data_type.split('_')[-1],data_type, m), 'rb'))
        assert len(X) == len(y)
        X_train.append(X)
        y_train.append(y)
    ...
    for m in range(1, test_count + 1):
        frames, _ = get_frames(predict_videoFile='./%s/marked_only_videos/%s_test_%d.markonly.mp4'% (data_type.split('_')[-1],data_type, m))
        assert frames
        X = prepare_data(frames)
        y = pickle.load(open('./%s/marked_only_data/y_%s_test_%d' % (data_type.split('_')[-1],data_type, m), 'rb'))
        assert len(X) == len(y)
...
def Predict_LRCN(data_type = 'foot_withdrawing' ,test_video='./withdrawing/marked_only_videos/foot_withdrawing_test_1.markonly.mp4'):
    ...
    #     f.write('\n')
    logger.info('Saving results to figure ./withdrawing/%s_timeLabel_%s.png'%(file_name,data_type))
    x_axis = [i/frame_rate for i in range(len(pred_scores))]
    plt.plot(x_axis,pred_scores)
    plt.ylim(-0.1, 1.1)
    plt.savefig('./withdrawing/%s_timeLabel_%s.png'%(file_name,data_type))
    # plt.show(block=True)
    # plt.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video', default='./withdrawing/marked_only_videos/foot_withdrawing_test_1.markonly.mp4',
                        help='path to test video')
    # parser.add_argument('--video', default='./test_marked_only/2700_1.markOnly.mp4',
    #                     help='path to test video')
