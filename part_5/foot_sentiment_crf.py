...
class CNN_CRF(nn.Module):
    ...
    def __init__(self,  tag_to_ix,device):
        ...
        self.fc1 = nn.Linear(16 * 6 * 13, 512)
        self.fc2 = nn.Linear(512, 32)
        self.fc3 = nn.Linear(32, self.tagset_size)
    ...
    for epoch in range(max_epoch):
        ...
        with torch.no_grad():
            ...
            if (correct/total)>best_acc and (model_out.argmax(axis=1).sum() !=0):
                torch.save(model, './save/cnn_model_%s'%data_type)
...
def Train_CNN_CRF():
    ...
    model = CNN_CRF(tag_to_ix,device)
    model.to(device)
    optimizer = optim.SGD(model.parameters(), lr=0.0002, weight_decay=1e-4)
    ...
    for epoch in range(10000):
        ...
        # Step 2. Get our inputs ready for the network, that is,
        # turn them into Tensors of word indices.
        i = random.randint(1, train_count)
        X_train = torch.tensor(pickle.load(open('./data/X_%s_train%d' % (data_type, i), 'rb'))).view(-1,3,240,416)
        y_train = torch.tensor(pickle.load(open('./data/y_%s_train%d' % (data_type, i), 'rb')))
        # print(X_train.size())
        # print(y_train.size())
        ...
        loss = model.neg_log_likelihood(X_train.to(device,dtype=torch.float), y_train.to(device,dtype=torch.long))
        ...
        # test
        if epoch % train_count == (train_count-1):
            print('[Epoch%d, Sample%d] loss: %.3f' %
                  (epoch + 1, i, loss_total /train_count))
            ...
            with torch.no_grad():
                ...
                total = 0.0
                for m in range(1,test_count+1):
                    X_test = torch.tensor(pickle.load(open('./data/X_%s_test%d' % (data_type, m), 'rb'))).view(-1, 3, 240,
                                                                                                                 416)
                    y_test = pickle.load(open('./data/y_%s_test%d' % (data_type, m), 'rb'))

                    # for j in tqdm(range(len(X_test))):
                    #     # if not vis is None and i == 0:
                    #     #     visdom_windows = plot_weights(model,visdom_windows,b,vocab,vis)
                    #     sample = X_test[j]
                    scores,model_out = model.forward(X_test.to(device,dtype=torch.float))
                    # print(model_out)
                    correct += (y_test == model_out).sum()
                    total+=len(y_test)
                accuracy = correct / total
                print("{}, {}/{}\n".format(accuracy, correct, total))
                if (correct/total)>best_acc:
                    torch.save(model, './save/cnn_crf_model_%s'%data_type)
                    best_acc = accuracy


    print('Finished Training')

def Predict_CNN_CRF(data_type = 'foot_pacing' ,test_video='./marked/foot_pacing_test1.mark.mp4'):
    ...
    print(file_name)
    frames = get_frames(predict_videoFile=test_video)
    X_test = torch.tensor(prepare_data(frames),dtype=torch.float)

    model = torch.load('./save/cnn_crf_model_%s'%data_type, map_location=device)
    # print(model)
    model.eval()
    ...
    model.to(device)
    with torch.no_grad():
        model_out = model(X_test.to(device, dtype=torch.float))[1]
        # for debugging
        # correct = (model_out.argmax(axis=1) == y.numpy()).sum()
        # total = len(model_out)
        # print("{}, {}/{}\n".format(correct / total, correct, total))
    pred_scores = model_out
    results_dic = {}
    results_dic[file_name] = []
    for i,score in enumerate(pred_scores):
        results_dic[file_name].append([str(i),str(score)])
    print('Saving results to frameLabel.json')
    with open('./frameLabel_crf.json','a') as f:
        json.dump(results_dic,f)
        f.write('\n')
    print('Saving results to figure %s_pred_label.png'%file_name)
    plt.plot(pred_scores)
    plt.savefig('%s_pred_label_crf.png'%file_name)
    # plt.show(block=True)
    # plt.close()
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 3:
        ...
        Predict_CNN_CRF(data_type = data_type,test_video=test_video)
    else:
        Predict_CNN_CRF()