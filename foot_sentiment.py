...
class CNN_CRF(nn.Module):
    ...
    def _get_cnn_features(self, pics):
        ...
        x = self.pool1(F.relu(self.conv1(pics)))
        x = self.drop_out(self.BN1(x))
...
def Predict_CNN(data_type = 'foot_pacing' ,test_video='./marked/foot_pacing_test1.mark.mp4'):
    ...
    model = torch.load('./save/cnn_model_%s'%data_type)
    model.eval()
    ...
    # print(results_dic)
    with open('./frameLabel.json','a') as f:
        json.dump(results_dic,f)
        f.write('\n')
    plt.plot(pred_scores)
    plt.savefig('%s_pred_label.png'%file_name)
...
if __name__ == '__main__':
    ...
    # Predict the results
    Predict_CNN(data_type = 'foot_turning_away' ,test_video='./marked/foot_turning_away_test.mark.mp4')